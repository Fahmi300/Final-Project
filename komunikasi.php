...
if (isset($_GET['targetID']) && isset($_GET['userID'])){
    ...
    $result = mysqli_query($db, "SELECT * FROM `user` ORDER BY us_id ASC LIMIT 1;");
                ...
                <?php
                    while ($res = mysqli_fetch_assoc($result)) {
                        if($res['us_id']!=$_GET['userID']){
                            echo "<div class='friend-box'>$res[us_username]<a href=\"komunikasi.php?targetID=$res[us_id]&userID=".$_GET['userID']."\">Chat</a></div>";
                        }
                    }
		        ?>
            </div>
